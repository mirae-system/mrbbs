<?php
/**
 * board/secret_verify.php — 비밀글 비밀번호 확인
 */
...
$post = $db->fetch(
    "SELECT id, slug, password FROM posts WHERE id = ? AND status = 'active'",
    array($postId)
);
...
// 인증 성공 — 세션에 저장 (60분)
$_SESSION['secret_verified_' . $postId]      = true;
$_SESSION['secret_verified_time_' . $postId] = time();
